CREATE OR REPLACE VIEW jc_seniorappointments AS SELECT hp.person_code,
hp.title,
...
, hp.USS_RECIPIENT
  ...
  WHERE pos.per_person_code = hp.person_code
   ...
   AND pos.control_context_id = cc.CONTROL_CONTEXT_ID
    ...
    AND ha.appointee_type in ('SENIOR_APPOINTMENT')
...
--
-- Describe the jc_seniorappointments view
--
DESCRIBE jc_seniorappointments;

--
-- How many people in this
--
SELECT COUNT(*) FROM jc_seniorappointments;
select appointee_type, control_context_id, count(*) as kount from jc_seniorappointments group by appointee_type, control_context_id order by 1;
