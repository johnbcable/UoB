CREATE OR REPLACE VIEW jc_honoraries AS SELECT hp.person_code,
hp.title,
...
, hp.USS_RECIPIENT
  ...
  WHERE pos.per_person_code = hp.person_code
   ...
   AND pos.control_context_id = cc.CONTROL_CONTEXT_ID
    ...
    AND ha.appointee_type in ('HONORARY')
...
--
-- Describe the jc_honoraries view
--
DESCRIBE jc_honoraries;

--
-- How many people in this
--
SELECT COUNT(*) FROM jc_honoraries;
select appointee_type, control_context_id, count(*) as kount from jc_honoraries group by appointee_type, control_context_id order by 1;
