CREATE OR REPLACE VIEW jc_uebmembers AS SELECT hp.person_code,
hp.title,
...
, hp.USS_RECIPIENT
  ...
  WHERE pos.per_person_code = hp.person_code
   ...
   AND pos.control_context_id = cc.CONTROL_CONTEXT_ID
    ...
    AND ha.appointee_type in ('UEB_MEMBER')
...
--
-- Describe the jc_uebmembers view
--
DESCRIBE jc_uebmembers;

--
-- How many people in this
--
SELECT COUNT(*) FROM jc_uebmembers;
select appointee_type, control_context_id, count(*) as kount from jc_uebmembers group by appointee_type, control_context_id order by 1;
