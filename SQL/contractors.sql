CREATE OR REPLACE VIEW jc_contractors AS SELECT hp.person_code,
hp.title,
...
, hp.USS_RECIPIENT
  ...
  WHERE pos.per_person_code = hp.person_code
   ...
   AND pos.control_context_id = cc.CONTROL_CONTEXT_ID
    ...
    AND ha.appointee_type in ('CONTRACTOR')
...
--
-- Describe the jc_contractors view
--
DESCRIBE jc_contractors;

--
-- How many people in this
--
SELECT COUNT(*) FROM jc_contractors;
select appointee_type, control_context_id, count(*) as kount from jc_contractors group by appointee_type, control_context_id order by 1;
