CREATE OR REPLACE VIEW jc_pensioners AS SELECT hp.person_code,
hp.title,
...
, hp.USS_RECIPIENT
  ...
  WHERE pos.per_person_code = hp.person_code
   ...
   AND pos.control_context_id = cc.CONTROL_CONTEXT_ID
    ...
    AND ha.appointee_type in ('PENSIONER')
...
--
-- Describe the jc_pensioners view
--
DESCRIBE jc_pensioners;

--
-- How many people in this
--
SELECT COUNT(*) FROM jc_pensioners;
select appointee_type, control_context_id, count(*) as kount from jc_pensioners group by appointee_type, control_context_id order by 1;
